...
5. The path is created immediately and named from the path template — `FROM↔TO` by default
   ...
   the two nodes.
6. Repeat for every pair (keep clicking to chain a route), then **Export…** and pick any
   combination of CSV, KML and KMZ (KMZ bundles any of the source file's own icon images
   along with it).
...
One **Export…** button (in the File menu) opens a dialog where you check any combination
of **CSV**, **KML** and **KMZ** and click **Export**. Picking a single format lets your
browser offer a native "save as" dialog so you choose the exact destination (where
supported — see below); picking more than one just downloads each file to your browser's
normal downloads location, since browsers only allow one such picker per click.

**CSV** writes one row per connection (or, with nothing connected yet, one row per node —
see above).

**KML** contains a `Nodes` folder (the original points) and a `Paths` folder with
...
where **KMZ** helps (see below). Built-in **Google-pin** icons (from Bulk style's
Icon option, see above) are unaffected either way, since they're a self-contained `data:`
URI, not a file reference.

**KMZ** writes the same KML but as a real `.kmz` archive, bundling in the exact icon
...
is equivalent to KML. The status bar reports how many images were bundled.

Whether you get a native save dialog also depends on your browser: it's only offered where
the File System Access API is available (e.g. desktop Chrome/Edge); elsewhere every export
downloads to the normal downloads location regardless of how many formats you picked.
